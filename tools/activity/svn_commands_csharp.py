...
class SvnDriver:
	...
	def summarize(self,root,t,module,rev):
		print "def summarize============================="
 		files = []
		...
		for _,file,_,state in revs:
			...
			state = state[0].upper()
			print state + "root:" + root + " =====comment by dkim=:" + path
			
			#added by Donghoon Kim
			if state[0] == "A":
				restore_file (path, rev)
...
#added by Donghoon Kim
def restore_file(path, rev):
	savedPath = os.getcwd()
	tmp_directory = path

	while len(tmp_directory) > 0:
		if not os.path.exists(tmp_directory):
			head = os.path.split(tmp_directory)
			if os.path.exists(head[0]):
				os.chdir(head[0])
				command =  'svn update -r ' + rev + " " + head[1]
				query = command
				print "query:" + query
				os.popen(query).read();
				return os.chdir(savedPath)
				
			else:
				tmp_directory = head[0]
		else:
			return
