...
{  
  ...
  public TrackerConnection(Map metainfo,byte[] hash,int port)
  {
    ...
    for(int i = 0 ; i < azureus.length ; i++)
    {
      peerId[i] = azureus[i];
    }
    for(int i = azureus.length ; i < 15 ; i++)
    {
      peerId[i] = (byte) 0;
    }
  ...
  private String update(String evt,boolean getResponse)
  {
    try {
