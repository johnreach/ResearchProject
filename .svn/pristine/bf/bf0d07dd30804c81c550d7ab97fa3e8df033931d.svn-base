...
public class DiskManager {
  ...
  public DiskManager(Map metaData, String path, PeerManager manager) {
  ...
  private void initialize() {
    ...
    if (test != null) {
      ...
      if (fullPath.lastIndexOf(rootPath)
      ...
      for (int i = 0; i < files.size(); i++) {
        ...
        for (int j = 0; j < fileList.size(); j++) {
          ...
          if (j != (fileList.size() - 1)) //are we the filename?
            ...
            try {
              pathBuffer.append(
                new String((byte[]) fileList.get(j), "ISO-8859-1"));
            } catch (UnsupportedEncodingException e) {
    ...
    lastPieceLength =
      (int) (totalLength - ((long) (nbPieces - 1) * (long) pieceLength));
          ...
          //use the rest of the file's space
            tempPieceEntry =
              new PieceMapEntry(
                tempFile.getFile(),
                fileOffset,
                (int) availableSpace //safe to convert here
  );
  ...
  private List buildPieceToFileList(
    List btFileList,
    int currentFile,
    int fileOffset,
    int pieceSize) {
  ...
  private boolean allocateFiles(String rootPath, List fileList) {
    ...
    for (int i = 0; i < fileList.size(); i++) {
      ...
      if (!(f.exists() && f.length() == length)) {
        ...
        ConfigurationManager config = ConfigurationManager.getInstance();
        int allocateNew = config.getIntParameter("Allocate New", 0);
        if (allocateNew == 1)
          clearFile(raf);
        newFiles = true;
      } else {
  ...
  public synchronized boolean checkPiece(int pieceNumber) {
    ...
    try {
      ...
      allocateAndTestBuffer.position(0);
      String testHash =
        new String(hasher.calculateHash(allocateAndTestBuffer), "ISO-8859-1");
      String correctHash =
        piecesHash.substring(pieceNumber * 20, pieceNumber * 20 + 20);
  ...
  public void dumpResumeDataToDisk() {
    ...
    try {
      torrent =
        new File(
          new String((byte[]) metaData.get("torrent filename"), "ISO-8859-1"));
    } catch (UnsupportedEncodingException e) {
      // TODO Auto-generated catch block
      e.printStackTrace();
    }
    ...
    while (buffer.hasRemaining()) {
      ...
      synchronized (raf) {
        try {
          ...
          int limit =
            buffer.position()
              + (int) ((raf.length() - tempPiece.getOffset())
        ...
        try{        
          ...
          for(int j = 0 ; j < nbPieces ; j++)
          {
            ...
            for (int k = 0; k < pieceList.size(); k++) {
              ...
              RandomAccessFile raf = tempPiece.getFile();
              if(raf == oldRaf) tempPiece.setFile(fileArray[i]);
            }
          }
        } catch (Exception e)
        {
